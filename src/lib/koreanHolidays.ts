/**
 * 한국 공휴일 목록 (2024-2030)
 * 원본: ijw-calander/constants_holidays.ts 의 STANDARD_HOLIDAYS
 *
 * 실제 쉬는 날만 포함 — 국경일, 공휴일, 대체공휴일.
 * 절기/잡절(경칩, 한식 등) 은 포함하지 않음.
 *
 * 연도가 늘어나면 여기 추가하면 됨.
 */
...
export const KOREAN_HOLIDAYS: KoreanHoliday[] = [
  ...
  { date: "2025-05-05", name: "어린이날 / 부처님 오신 날" },
...
export function getHolidaysForYear(year: number): KoreanHoliday[] {
  const prefix = `${year}-`;
  return KOREAN_HOLIDAYS.filter((h) => h.date.startsWith(prefix));
}
