...
-- 새 ID 로 다시 채워짐.
--
-- ⚠ Supabase SQL Editor 가 statement 별로 분리 실행하는 환경 호환을 위해 전체를
-- 단일 DO 블록으로 wrap (= 단 하나의 SQL statement). 모든 작업이 한 트랜잭션으로 atomic.
--
-- ⚠ Idempotent: 이미 적용되었으면 마이그레이션 대상 0 건 → no-op.
...
BEGIN
  ...
  SELECT COUNT(*) INTO remap_count FROM _migration_025_id_remap;
  RAISE NOTICE '[migration_025] virtual_students 마이그레이션 대상: % 건', remap_count;

  IF remap_count = 0 THEN
    DROP TABLE _migration_025_id_remap;
    RAISE NOTICE '[migration_025] 마이그레이션 대상 없음. 종료.';
    RETURN;
  END IF;
